...
### Forecasting
  ...
  ## Forecast Comparison
    ...
    # MSE, RMSE, MAPE, & MAPA
    MSE <- mean((comparison$Close - comparison$lstm_forecast)^2)
    RMSE <- sqrt(MSE)
    MAPE <- (1/length(comparison$Close)) * sum(abs(comparison$Close - comparison$lstm_forecast)/comparison$Close) * 100
    MAPA <- 100 - MAPE
